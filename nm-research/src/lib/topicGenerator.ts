...
const LEVEL_PROFILE: Record<ExpertiseLevel, { label: string; deep: boolean }> = {
  undergrad: { label: "Undergraduate", deep: false },
  postgrad: { label: "Postgraduate", deep: false },
  phd: { label: "PhD", deep: true },
  postdoc: { label: "Postdoctoral", deep: true },
};
...
export function generateTopics(input: TopicInput, count = 4): GeneratedTopic[] {
  ...
  const subjects = domain.subjects.map((s) => s.name);
  ...
  for (let i = 0; i < n; i++) {
    ...
    topics.push({
      ...
      angle,
      why: `A focused topic within ${focusTitle}, sized for ${LEVEL_PROFILE[input.level].label.toLowerCase()} scope and aligned with ${subj} in ${domain.title}.`,
    });
  }
